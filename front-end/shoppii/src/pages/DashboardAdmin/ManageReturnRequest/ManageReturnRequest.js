...
export default function ManageReturnRequest() {
    ...
    const fetchList = async () => {
        setLoading(true);
        try {
            const res = await axios.get("http://localhost:9999/api/admin/return-requests?skipAuth=true");
    ...
    const handleUpdate = async () => {
        setSaving(true);
        try {
            await axios.put(`http://localhost:9999/api/admin/return-requests/${selected._id}?skipAuth=true`, {
                status
            });
            await fetchList();
            handleClose();
