...
export default function AddProduct({ onAdded }) {
  ...
  // Fetch categories
  React.useEffect(() => {
    axios.get('http://localhost:9999/api/admin/categories?skipAuth=true')
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      const result = await axios.post('http://localhost:9999/api/admin/products?skipAuth=true', requestBody, {
        headers: {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const result = await axios.post('http://localhost:9999/api/admin/categories?skipAuth=true', requestBody, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
