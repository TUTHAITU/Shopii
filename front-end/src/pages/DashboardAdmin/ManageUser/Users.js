...
export default function Users({ users: initialUsers, onUserUpdated }) {
  ...
  // Handle locking or unlocking a user
  const handleLockUnlockUser = async (user, action) => {
    if (!user) return;
  ...
  const handleUserAction = (action) => {
    if (action === 'edit' && selectedUser) {
      ...
      handleLockUnlockUser(selectedUser, 'lock');
    } else if (action === 'unlock' && selectedUser) {
      handleLockUnlockUser(selectedUser, 'unlock');
    }
  ...
  return (
    <React.Fragment>
      ...
      >
          ...
          {viewingUser && (
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sx={{ textAlign: 'center', mb: 2 }}>
                <Typography variant="h6">{viewingUser.username}</Typography>
                <Typography variant="body2" color="text.secondary">{viewingUser.email}</Typography>
                <Box sx={{ display: 'flex', justifyContent: 'center', mt: 1 }}>
                  <UserStatusChip status={viewingUser.action || "unknown"} />
                </Box>
              </Grid>
        ...
        <DialogActions>
          <Button onClick={() => setViewingUser(null)} color="primary">
            Close
          </Button>
          <Button 
            onClick={() => {
